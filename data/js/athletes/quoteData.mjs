import athletes from "./athletes.mjs";
...
const quoteData = {
    ...
    initialize(athletes) {
        if (Array.isArray(athletes) && athletes.length > 0) {
            this.quotes = athletes.map(({ quote }) => quote);
            this.authors = athletes.map(({ author }) => author);
            this.quoteOptions = athletes.map(item => item.options);
            this.removedQuotes = [];
        } else {
            this.quotes = [];
            this.authors = [];
