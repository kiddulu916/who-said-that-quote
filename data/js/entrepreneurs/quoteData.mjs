...
const quoteData = {
    ...
    initialize(entrepreneurs) {
        if (Array.isArray(entrepreneurs) && entrepreneurs.length > 0) {
            this.quotes = entrepreneurs.map(({ quote }) => quote);
            this.authors = entrepreneurs.map(({ author }) => author);
            this.quoteOptions = entrepreneurs.map(item => item.options);
            this.removedQuotes = [];
        } else {
            this.quotes = [];
            this.authors = [];
