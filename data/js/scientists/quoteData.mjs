import scientists from "./scientists.mjs";
...
const quoteData = {
    ...
    initialize(scientists) {
        if (Array.isArray(scientists) && scientists.length > 0) {
            this.quotes = scientists.map(({ quote }) => quote);
            this.authors = scientists.map(({ author }) => author);
            this.quoteOptions = scientists.map(item => item.options);
            this.removedQuotes = [];
        } else {
            this.quotes = [];
            this.authors = [];
